defmodule Tai.VenueAdapters.Deribit.Product do
  ...
  def build(instrument, venue_id) do
    ...
    min_trade_amount =
      instrument.min_trade_amount |> Decimal.cast() |> elem(1) |> Decimal.normalize()

    {:ok, tick_size} = instrument.tick_size |> Decimal.cast()
    contract_size = instrument.contract_size |> Decimal.cast() |> elem(1) |> Decimal.normalize()
    {:ok, maker_fee} = instrument.maker_commission |> Decimal.cast()
    {:ok, taker_fee} = instrument.taker_commission |> Decimal.cast()
