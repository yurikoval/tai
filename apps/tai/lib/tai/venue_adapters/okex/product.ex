defmodule Tai.VenueAdapters.OkEx.Product do
  ...
  defp build_product(args) do
    ...
    {:ok, price_increment} = args |> Keyword.fetch!(:venue_price_increment) |> Decimal.cast()
    {:ok, size_increment} = venue_size_increment |> Decimal.cast()
    {:ok, min_size} = args |> Keyword.get(:venue_min_size, venue_size_increment) |> Decimal.cast()
    {:ok, value} = args |> Keyword.fetch!(:value) |> Decimal.cast()
